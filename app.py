...
Fixes: radio label, use_container_width, gender bias removed,
       real stats, bright nav/tab text, PDF download, bureau removed,
       security token removed, hero text visible.
"""

import os, sys, pickle, io, textwrap
import pandas as pd
import numpy as np
...
section[data-testid="stSidebar"] label{
    ...
    color:#C8DCC8!important;text-transform:none!important;
    letter-spacing:0!important;padding:9px 12px!important;
    border-radius:8px!important;display:block!important
}
section[data-testid="stSidebar"] label:hover{
    background:#1A2E1A!important;color:#E8F5E8!important
}
/* selected radio option */
section[data-testid="stSidebar"] [data-baseweb="radio"] [aria-checked="true"] ~ div label{
...
header[data-testid="stHeader"]{
    background:#0D120D!important;border-bottom:1px solid #1A2E1A!important
}
...
/* Form field labels — bright so they're readable */
label,.stSelectbox label,.stNumberInput label,.stTextInput label{
    font-size:12px!important;font-weight:600!important;
    color:#7EC87E!important;text-transform:uppercase!important;
    letter-spacing:.06em!important
}

/* Tabs — make ALL tab text bright and readable */
[data-testid="stTabs"] button{
    color:#7EC87E!important;font-size:13px!important;
...
[data-testid="stTabs"] [role="tabpanel"]{
    background:#0A0E0A!important;padding-top:16px!important
}
...
[data-testid="stMetricLabel"]{
    color:#7EC87E!important;font-size:11px!important;
...
thead th{
    background:#0F1F0F!important;color:#7EC87E!important;font-size:11px!important;
...
[data-testid="stDownloadButton"]>button{
    background:#0F2010!important;color:#22C55E!important;
    border:1px solid #22C55E55!important;border-radius:8px!important;
...
# ─────────────────────────────────────────────────────────────────────
def generate_eda_pdf(df: pd.DataFrame, plot_dir: str) -> bytes:
    ...
    h2_style    = ParagraphStyle("h2", parent=styles["Heading2"],
                                  fontSize=13, spaceAfter=4, textColor=colors.HexColor("#16A34A"))
    body_style  = ParagraphStyle("body", parent=styles["Normal"],
                                  fontSize=10, spaceAfter=4, textColor=colors.HexColor("#333333"))

    story = []
    story.append(Paragraph("German Credit Risk — EDA Report", title_style))
    story.append(Paragraph(f"Generated from {len(df)} training records", body_style))
    ...
    tdata = [
        ...
        ["Features",       "7 (Job, Housing, Saving accounts, Age, Credit amount, Duration, Sex)"],
    ]
    ...
    insights = [
        f"• Good credit applicants are on average {good_age:.1f} years old vs {bad_age:.1f} for bad credit.",
        f"• Good credit applicants request ₹{good_cr:,.0f} on average vs ₹{bad_cr:,.0f} for bad credit.",
        f"• Good credit loans last {good_dur:.1f} months on average vs {bad_dur:.1f} months for bad credit.",
        f"• Saving accounts is the most predictive feature — high savers almost always repay.",
        f"• Longer loan duration consistently correlates with higher default risk.",
        f"• Gender (Sex) was excluded from model training to ensure fairness.",
    ]
    for ins in insights:
    ...
    plot_map = [
        ...
        ("04_duration_analysis.png",   "Duration Analysis"),
        ("09_correlation_heatmap.png", "Correlation Heatmap"),
        ("feature_importance.png",     "Feature Importance"),
        ("confusion_matrix.png",       "Confusion Matrix"),
    ]
    for fname, title in plot_map:
        fpath = os.path.join(plot_dir, fname)
        if os.path.exists(fpath):
            story.append(PageBreak())
            story.append(Paragraph(title, h2_style))
            story.append(Spacer(1, 0.2*cm))
            img = Image(fpath, width=15*cm, height=9*cm)
            story.append(img)
...
with st.sidebar:
    ...
    st.markdown("""
    <div style="margin-top:20px;padding:12px 14px;background:#0A120A;border:1px solid #1A2E1A;
        border-radius:10px">
        ...
        <div style="font-size:11px;color:#3D5A3D;margin-top:4px">
            Fair ML Model · Gender-Neutral
        </div>
    </div>""", unsafe_allow_html=True)
...
if "Home" in page:
    ...
    # Hero — extra top padding so text clears the Streamlit header bar
    st.markdown("""
    ...
    with c4: stat_card("⚖️", "Neutral",    "Gender Fairness", "#22C55E")
    ...
    with col_info:
        ...
        st.markdown(f"""
        <div style="background:#0A1F0A;border:1px solid #1A3A1A;border-radius:10px;padding:14px 16px;margin-top:4px">
            <div style="font-size:12px;color:#7EC87E;line-height:1.6">
                ⚖️ <strong style="color:#E8F5E8">Fairness Note:</strong>
                Gender (Sex) is <strong style="color:#22C55E">excluded</strong> from model predictions.
...
# ═══════════════════════════════════════════════════════════════════
    ...
    if submitted:
        # Build input — Sex NOT passed to model
        ...
        section_title("AI Decision Explanation",
                      "SHAP values — each bar shows how much each factor pushed the decision")
        ...
        <div style="background:#0A1F0A;border:1px solid #1A3A1A;border-radius:10px;padding:16px 18px;margin-bottom:16px">
            <div style="font-size:13px;color:#A0C8A0;line-height:1.7">
                ...
                The model analysed <strong style="color:#22C55E">6 financial features</strong> for
        ...
        raw_vals = {
            "Job": {0:"Unskilled(NR)",1:"Unskilled(R)",2:"Skilled",3:"HQ"}[job],
            "Housing": housing, "Saving accounts": saving,
            "Age": age, "Credit amount": f"₹{credit_amount:,}", "Duration": f"{duration}mo"
        }
        for r in reasons:
        ...
        summary = pd.DataFrame({
            ...
            "Value": [name or "—", f"{gender} ⚖️",
                      {0:"Unskilled(NR)",1:"Unskilled(R)",2:"Skilled",3:"Highly Qualified"}[job],
...
# ═══════════════════════════════════════════════════════════════════
    ...
    # Tabs — labels are non-empty strings
        ...
        if not available:
            ...
            for i in range(0, len(available), 2):
                ...
                for col, (k, path) in zip([c1, c2], available[i:i+2]):
                    with col:
                        st.markdown(
                            f"<div style='font-size:12px;font-weight:600;color:#7EC87E;"
    ...
    with tab2:
        section_title("Interactive Distribution Analysis",
                      "Select any feature to see how it relates to credit risk")

        feat_sel = st.selectbox(
            "Select Feature",
            ["Age", "Credit amount", "Duration", "Job", "Housing", "Saving accounts"],
            key="eda_feat_sel"
        )
        ...
        if numeric:
            ...
            axes[1].boxplot([bv, gv], tick_labels=["Bad", "Good"],
                patch_artist=True,
                boxprops=dict(facecolor="#1A2E1A", color="#22C55E"),
                medianprops=dict(color="#22C55E", linewidth=2),
                whiskerprops=dict(color="#3D5A3D"),
                capprops=dict(color="#3D5A3D"),
            ...
            axes[1].set_ylabel(feat_sel, color="#7EC87E")
            axes[1].tick_params(colors="#7EC87E")
        else:
            ...
            axes[0].legend(facecolor="#1A2E1A", labelcolor="#C8DCC8")
            axes[0].set_ylabel("Count", color="#7EC87E")
...
# ═══════════════════════════════════════════════════════════════════
    ...
    <div style="margin:16px 0 24px">
        ...
        <p style="font-size:13px;color:#5A7A5A;margin:0">
            Feature importance, fairness audit, and decision logic.</p>
    </div>""", unsafe_allow_html=True)
    ...
    section_title("Feature Importance",
                  "Mean decrease in impurity — how much each feature contributed to predictions")
    imp = pd.Series(model.feature_importances_, index=FEAT_COLS).sort_values(ascending=True)
    median_imp = imp.median()

    fig, ax = plt.subplots(figsize=(8, 3.5))
    fig.patch.set_facecolor("#111811"); ax.set_facecolor("#111811")
    colors_bar = ["#22C55E" if v >= median_imp else "#1A3A1A" for v in imp.values]
    bars = ax.barh(imp.index, imp.values, color=colors_bar, edgecolor="#0A0E0A", height=0.55)
    ax.set_xlabel("Importance Score", color="#7EC87E", fontsize=11)
    ax.tick_params(colors="#7EC87E")
    for s in ax.spines.values(): s.set_color("#1A2E1A")
    for bar, val in zip(bars, imp.values):
        ax.text(val + 0.002, bar.get_y() + bar.get_height()/2,
                f"{val:.3f}", va="center", color="#A0C0A0", fontsize=9)
    ...
    # Fairness audit
    section_title("Fairness Audit", "Demographic parity — approval rates across gender groups")
    st.markdown("""
    <div style="background:#0A1F0A;border:1px solid #1A3A1A;border-radius:10px;padding:14px 18px;margin-bottom:16px">
        <div style="font-size:13px;color:#7EC87E;line-height:1.7">
            ...
            A male and female with identical financial profiles will receive <strong style="color:#22C55E">exactly the same prediction</strong>.
            This is the strongest possible fairness guarantee — the model literally cannot see gender.
        </div>
    </div>""", unsafe_allow_html=True)
    ...
    try:
        ...
        with fc3: stat_card("⚖️", f"{dp_diff:.3f}", "Parity Gap",
                            "#22C55E" if dp_diff < 0.15 else "#EF4444")

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        if dp_diff < 0.15:
            st.info(f"✅  Demographic parity gap = {dp_diff:.3f}. Remaining gap reflects real differences in financial profiles in the dataset, not model bias — gender was excluded from predictions.")
        else:
...
# ═══════════════════════════════════════════════════════════════════
    ...
    <div style="font-size:13px;color:#7EC87E;line-height:1.75">
        ...
        to guarantee fairness — the strongest possible fairness guarantee. Built for FinTech
        environments where explainability and compliance are mandatory.
    </div>""", "#22C55E")
    ...
    stack = [
        ...
        ("Report",          "ReportLab — PDF EDA report generation"),
    ]
    c1, c2 = st.columns(2, gap="large")
...
python src/data/load_and_clean.py    # download + EDA
python eda.py                         # generate all 11 plots
python src/models/train_model.py      # train model
streamlit run app.py                  # launch dashboard""", language="bash")

    section_title("Update on GitHub + Streamlit Cloud")
    st.code("""git add app.py src/models/explain.py src/data/preprocess.py models/model.pkl
git commit -m "fix: gender bias removed, UI improved, PDF report added"
git push
